...
- [ ] Copy lane 1 and lane 3 screenshots, plus the adoption run with its before-and-after row counts, into `/Users/smurphnerd/projects/hanzimind-evidence/review/P4-MIGRATE-review-adopt.png`. Lane 7's screenshot has no source now that its box is deferred to the operator's cutover gate, and this is the same evidence without Neon.
- [ ] Record a 30 to 60 second video of the cutover on a data-bearing lane, or a `script(1)` terminal transcript where no recorder exists, which is the case on this machine. Save it as `/Users/smurphnerd/projects/hanzimind-evidence/review/P4-MIGRATE-review.txt`. No package will be installed to make the artifact prettier, and the only recorder present films the operator's own display. Not a Neon branch, since that is deferred. Boot the lane from a worktree at trunk, whose `lane-up.sh` still uses `drizzle-kit push`, so the database has production's shape with no journal and nothing hand-edited about the bookkeeping under test.
...
- [ ] Add indexes on `memoryAids.vocabItemId`, `suggestions(createdById, createdAt)`, `suggestions.status`, `userDecks.deckId`, `userVocabItems.memoryAidId`, `deckVocabItems.vocabItemId`, `vocabItems(disabled, vocabType)`, and no index for the ILIKE search columns, which is not deliverable, because a lower-cased b-tree cannot serve a leading wildcard, and a trigram index needs `create extension pg_trgm`, which `db:push` does not emit, so declaring one breaks a push on any unprepared database. Measured rather than assumed, and the search seq-scans 9,642 rows in 7 ms and the planner ignores the index.
- [ ] Add `onDelete: "cascade"` to `userVocabItems`, `userDecks`, `memoryAids`, `userVocabSynonyms` and `userStudyProgress` user references, plus `deckVocabItems.deckId`, since that table has no user reference at all. Leave `decks.createdById` without a cascade so a published deck cannot vanish under the learners studying it, and match P4-AUTH's hook, which deletes an authored deck no other learner has saved and refuses the account deletion only when one has.
...
- [ ] Lane 6. Grep for `includeConstituents` and for `isConstituent` across `src`, `scripts`, `e2e` and the root markdown. Save `index-column-gone.png`. Pass when both return zero there. Repo-wide can never be zero, because the plan and findings files name both columns as the record.
- [ ] Lane 7. Save a deck and study one card. Save `index-study.png`. Pass when both persist.
- [ ] Lane 8. Submit twelve suggestions quickly, since `SUGGESTION_RATE_LIMIT` is 10 an hour and three never trips it. Save `index-ratelimit.png`. Pass when the rate limit still trips at the same count as trunk.
...
- [ ] Lane 9. Run 20 answers and compare the served order to a trunk lane seeded identically. Save `progress-sequence-parity.png`. Pass when identical. **HSK 1 cannot satisfy this and the verifier proved it.** From one restored fixture, trunk disagreed with itself, sharing nine cards and then diverging, because the deck has ties the selector breaks at random. So the lane must first construct a tie-free state of ten due `understanding` cards whose `(minLevel, vocabTypePriority, decompositionLength)` triples are pairwise distinct, verified distinct in SQL before any answer is submitted, and compare on that. A parity box on a deck that is not deterministic tests the tiebreak, not the change.
...
- [ ] Lane 3. Stop Postgres and load `/dictionary/人`, a detail page. Save `obs-error-route.png`. Pass when the failure card shows a request id. This box has now been wrong twice. `/decks` uses `useQuery` and renders an inline error; `/dictionary` uses `useQuery` gated on a non-empty search term, so it returns 200 and an ordinary empty page with nothing to screenshot. Note also what a pass here proves, which is the local `ErrorBoundary` in `src/components/error-boundary.tsx` rather than `src/app/error.tsx`, since verification could reach neither `error.tsx` nor `global-error.tsx` by any user path.
...
A JSONB progress map instead of rows. P4-PROGRESS took the rows. The JSONB map is the cheaper migration, one `UPDATE` building the object from the four pairs, no new table, no second query, and the deck read stays exactly one row per item. What it cannot do is be indexed or constrained. `next_at` inside a JSONB object cannot answer "what is due" without a functional index per key, the four study types are not enforced, and a level is whatever JSON says it is. The rows also make the sparse case honest, a type that has never been answered simply has no row, where a JSONB map has to spell out four entries or leave the reader guessing what a missing key means. The cost the rows do carry is a second query on the deck path, and joining the per-type rows would repeat every card column up to four times and undo the projection work P3-STUDY-SVC did, so `getNextVocabItem` runs the card query and the progress query in parallel instead.

One row per type, but not four rows for every item. A pair at level 0 with a null due time is what a missing row already means, so the copy skips those, and writing them would be 1,592 rows for one learner on HSK 1 where 103 carry a fact. Level 0 WITH a due time is an answer got wrong, is a different fact, and is written. The verification enforces the distinction rather than trusting it, it accepts a missing row only where the legacy pair was that exact default.

Exiting 0 when the migration has already lost the data, in P4-PROGRESS. If `pnpm db:push` runs before the copy, the eight columns and every level in them are gone, `--down` has no rows to restore from, and the script's "already keeps progress in `user_study_progress`, nothing to do" was both true and the worst possible thing to say. The state has an unambiguous signature, nobody's progress in the new table, yet `user_vocab_items` rows marked seen, and `seen` is the one column this migration never moves, so the script refuses and names a snapshot restore instead. The same guard covers `--down`, whose reset would otherwise zero the columns it was run to recover. Its blind spot is a learner who has only ever been shown introductions, which the refusal says out loud rather than trying to resolve. Verification found this; recorded as finding 66.

A migration with no way back, in P4-PROGRESS. The reshape drops eight columns off a table holding a real learner's history, and `db:push` leaves no artefact to revert. So the script carries both directions. `--down` re-adds the columns with the definitions they had at `a391426`, fills them from the rows, and checks the result with the same query the forward direction commits on, which is symmetric and so proves both. Verified as a round trip on lane 8, original, copy, push, restore, with the restored snapshot byte-identical to the original and trunk's own code then studying on it. It is not a substitute for a `pg_dump` before the copy, and it can only restore what the rows still hold.

Locking the progress row rather than its parent, in P4-PROGRESS. The obvious move after the reshape is to point P3-STUDY-SVC's `SELECT ... FOR UPDATE` at the row that now holds the level. It silently undoes that PR's fix, because a study type has no progress row until its first answer, and a lock on a row that is not there serialises nothing, so two concurrent first answers both read level 0 and one is lost. The lock stays on the `user_vocab_items` row, which always exists, and is taken by the `UPDATE ... SET seen = true` that had to run anyway.
...
better-auth stays at 1.6.30, which is a minor behind rather than a major. It is excluded on purpose while P4-ORACLE's sign-up redesign is unmerged: an upgrade underneath an unreviewed security fix makes both unreviewable. Take it after that branch lands, with the sign-up oracle tests as the gate.
