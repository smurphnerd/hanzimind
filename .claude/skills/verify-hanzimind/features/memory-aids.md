...
- **Add a curated aid.** In the manage dialog, run `find` with `textarea with placeholder Add a curated memory aid…` and `form_input` with value `Curated: a person standing`. Click `Add aid`. The textarea clears and the new aid appears in the list with no `Private` badge. Read it back with `psql "$DATABASE_URL" -c "select public from memory_aids where memory_aid = 'Curated: a person standing';"`. `public` is `t`. The aid body is unbounded: a pasted 5,000-character aid is accepted and stretches the entry page past 60,000px, so keep test aids short.
