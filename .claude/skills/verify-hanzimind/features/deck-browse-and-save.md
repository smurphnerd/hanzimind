...
- **Database proof.** Run `psql "$DATABASE_URL" -c "select user_id, deck_id, reading_enabled, listening_enabled, understanding_enabled, writing_enabled from user_decks where deck_id = '<id>';"`. Exactly one row exists, `user_id` is the learner's `users.id`, and the four mode columns match the last dialog or `curl` values. There is no `include_constituents` column: P4-INDEX dropped it, since constituents are always studied and nothing ever read the flag. Then run `psql "$DATABASE_URL" -c "select count(*) from user_vocab_items where user_id = (select id from users where email = 'verify@hanzimind.test');"`. The count equals `select count(*) from deck_vocab_items where deck_id = '<id>'`, because saving seeds one progress row per deck item.
