...
- **Empty bucket.** With every report open, click `Resolved`. The list shows `Nothing to review` and `No suggestions in this bucket.` The sleeping mascot also puts a bare `z` into `get_page_text` between the bucket counts and the title, so match those two sentences rather than the whole block. With no open reports the `Open` bucket reads `No open suggestions — the garden is tidy.`
