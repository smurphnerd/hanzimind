...
- The rows named below exist from the seed. `亻` is a meaning-only component with an empty stored reading; its `Reading` field shows `No reading`. `艮` is a phonetic component with reading `gěn`. `人` is a character.
...
- **Phonetic on a meaning-only component.** Run `find` with query `switch labeled Teach the sound of 亻`, then `computer` `left_click`. A toast reads `Updated 亻` and the switch is on. The `Reading` cell for `亻` still shows `No reading`, unchanged. Run `navigate` to `http://localhost:PORT/dictionary/亻`. The entry header badge reads `Meaning + sound` and no reading is shown, because the stored reading is empty. Toggle the switch off again on `/admin/vocab` and reload the entry. The badge reads `Meaning only`, the reading is gone, and the header text starts `A part used to build other characters`.
- **Reading and phonetic are independent.** After the two bullets above, run `get_page_text` on `/admin/vocab` with `亻` in view. The `Reading` cell for `亻` is `No reading` and its `Phonetic` switch is off. Editing one never changed the other.
...
- **API edit.** Sign in as the admin with `curl` into `admin-jar.txt` and reuse it. Read the row id with `psql "$DATABASE_URL" -Atc "select id from vocab_items where vocab_item = '亻'"`. Run `curl -s -i -b admin-jar.txt -H 'content-type: application/json' -X POST http://localhost:PORT/api/rpc/admin/updateVocabItem -d '{"json":{"id":"<id>","translation":"person (side form)","phonetic":false}}'`. HTTP `200` and a body whose `json` object has `vocabItem` `亻`, `translation` `person (side form)`, `phonetic` `false`, `disabled` `false`, and the unchanged `pinyin` `""` (the seeded lane stores an empty reading for `亻`).
