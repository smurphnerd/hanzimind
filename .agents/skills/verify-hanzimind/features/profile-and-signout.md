...
- **Sign out through the API.** Prove the same from the terminal with the cookie jar from `sign-in.md`. Run `curl -s -o /dev/null -w "%{http_code}" -b jar.txt -H "content-type: application/json" -d '{}' http://localhost:PORT/api/auth/sign-out`. The status is `200`. A following `curl -s -b jar.txt http://localhost:PORT/api/auth/get-session` returns `null`.
