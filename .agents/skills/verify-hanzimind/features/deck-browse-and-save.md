...
- `1 learner` on the card counts `user_decks` rows for that deck. It is the fastest visible proof of a save without leaving `/decks`.
...
- The depth control only renders when the deck has at least two levels. An `HSK 1` built from `deck-create.md` has parts inside it, so it has more than one level and the control appears.
