...
```sh
.Codex/skills/verify-hanzimind/scripts/lane-up.sh 3
```
...
```sh
.Codex/skills/verify-hanzimind/scripts/doctor.sh 3
```
...
Headless. Only one worker at a time can drive `Codex-in-chrome`, because every worker shares one extension tab group. Every other browser lane drives headless Chromium through Playwright, which the repo ships as its e2e suite:
...
```sh
.Codex/skills/verify-hanzimind/scripts/perf-probe.mjs --port 3003 --rpc vocab/search \
...
```sh
.Codex/skills/verify-hanzimind/scripts/lane-down.sh 3
```
...
Before committing from a checkout that ran a lane, check `git status`. Next 16.3.4 and later write a `nextjs-agent-rules` block into `AGENTS.md` when `next dev` detects an agent. That block is not part of any PR; restore the file with `git checkout AGENTS.md`.

## Helpers

All under `.Codex/skills/verify-hanzimind/scripts/`. The four commands are executable. `lane-lib.sh` is mode 644 because it is sourced, never run.
...
| `prefetch-model.ts` | `pnpm exec tsx .Codex/skills/verify-hanzimind/scripts/prefetch-model.ts` | Download the semantic model into the app's cache; `lane-up.sh` runs it when the model is missing. |
